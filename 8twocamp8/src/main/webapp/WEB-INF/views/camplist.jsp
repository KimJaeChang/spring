   ...
   <div class="table-responsive">
      <table class="table">
         ...
         <c:forEach var="camp" items="${camplist }">
            <tr>
               <td>${camp.cno}</td>
               <td><a href="detail?bno=${camp.cno}"> ${camp.cname} </a></td>
               <td>${camp.oname}</td>
               <td>${camp.ckinds}</td>
               <td><span class="badge">${camp.caddress}</span></td>
            </tr>
         </c:forEach>
         <tr>
   ...
   <div id="text-center">
      <ul class="pagination">
         <c:if test="${pageMaker.prev}">
            <li><a href="${contextPath}/camp/camplist?page=${pageMaker.startPage-1}">이전</a></li>
         </c:if>
         <c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
            <li <c:out value="${pageMaker.criteria.page == idx ?'class=active' : ''}"/>>
            <a href="${contextPath}/camp/camplist?page=${idx}">${idx}</a>
            </li>
         </c:forEach>
         <c:if test="${pageMaker.next && pageMaker.endPage>0}">
            <li><a href="${contextPath}/camp/camplist?page=${pageMaker.endPage+1}">다음</a></li>
         </c:if>
      </ul>
   </div>
